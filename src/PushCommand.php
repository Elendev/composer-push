...
class PushCommand extends BaseCommand
{
        ...
        $this->setName('push')
            ...
            ->setDefinition([
                new InputArgument(
                    ...
                    'The package version, if not set take composer.json version',
                ),
                new InputOption(
                    'name',
                    null,
                ...
                new InputOption(
                    ...
                    null,
                    InputArgument::OPTIONAL,
                    'URL to the distant repository',
                ),
                new InputOption(
                    'type',
                    null,
                    InputArgument::OPTIONAL,
                    'Type of the distant repository (default: nexus, available: [' .
                        implode(', ', array_keys(self::PROVIDER_TYPES)) .
                        '])',
                ),
                new InputOption(
                    self::REPOSITORY,
                    null,
                    InputArgument::OPTIONAL,
                    'which repository to save, use this parameter if you want to place development version and production version in different repository',
                ),
                new InputOption(
                    'username',
                    null,
                ...
                new InputOption(
                    ...
                    'Ignore .gitattrbutes export-ignore directories when creating the zip',
                ),
                new InputOption(
                    'ignore-by-composer',
                    null,
                    InputOption::VALUE_NONE,
                    'Ignore composer.json archive-exclude files and directories when creating the zip',
                ),
                new InputOption(
                    'src-type',
                    null,
                    InputArgument::OPTIONAL,
                    'The source type (git/svn,...) pushed on composer on distant Nexus repository',
                ),
                new InputOption(
                    'src-url',
                    null,
                    InputArgument::OPTIONAL,
                    'The source url pushed on composer on distant Nexus repository',
                ),
                new InputOption(
                    'src-ref',
                    null,
                    InputArgument::OPTIONAL,
                    'The source reference pushed on composer on distant Nexus repository',
                ),
                new InputOption(
                    'keep-vendor',
                    null,
                    InputOption::VALUE_NONE,
                    'Keep vendor directory when creating zip',
                ),
                new InputOption(
                    'keep-dot-files',
                    null,
                    InputOption::VALUE_NONE,
                    'Keep dots files/dirs when creating zip',
                ),
                new InputOption(
                    'ssl-verify',
                    null,
                    InputOption::VALUE_OPTIONAL,
                    'Enable (true) or disable (false) the SSL verification',
                ),
                new InputOption(
                    'access-token',
                    null,
                    InputOption::VALUE_OPTIONAL,
                    'Access Token to get authenticated with',
                ),
            ])
            ->setHelp(
                <<<EOT
    ...
    protected function execute(
        ...
        if (
            ...
            if (
                ...
                throw new InvalidArgumentException(
                    'Source reference parameters are not complete, you should set all three parameters (type, url, ref) or none of them, please check',
                );
            }
        }
        ...
        if (empty($this->configuration->getVersion())) {
            throw new InvalidArgumentException(
                'No version found, you chould either provide version argument in the command or add version in composer.json',
            );
        }

        $packageName = $this->configuration->getPackageName();

        $subdirectory = strtolower(
            preg_replace(
                '/[^a-zA-Z0-9_]|\./',
                '-',
        ...
        try {
            ...
            $this->getIO()->write(
                'Archive correctly pushed to the ' .
                    ucfirst($this->configuration->getType()) .
                    ' server',
            );
    ...
    private function getProvider($type = null)
    {
        ...
        if (!array_key_exists($type, self::PROVIDER_TYPES)) {
            throw new \InvalidArgumentException(
                "Provider of type $type does not exist",
            );
        }
        ...
        if (!class_exists($class)) {
            throw new \RuntimeException(
                "Provider of type $type: class $class not found",
            );
        }
